...
- **Claims are the authoritative runtime ownership; `assigned_agent_id` is advisory.** A `node_claims` row (TTL lease) is the source of truth for "who is working on this task now" — `suggestNextTasks`/`claim_next_task` exclude actively-claimed nodes. The `assigned_agent_id`/`assigned_agent_at` columns are a display/intent hint only (who it's *meant* for) and must NOT be treated as a lock. Don't gate work on assignment; gate on claims.
- **Dependency edge vocabulary is `blocks` + `relates_to`** (node→node), enforced via `validation/dependencyTypes.js` (`normalizeNodeDependencyType` maps legacy `requires`/`informs`/`related_to`; rejects unknowns). `achieves` is the separate node→GOAL contribution edge, created only via the goal achievers routes / plan-link cascade — not accepted on `/plans/:id/dependencies`. Only `blocks` drives critical-path / scheduling traversal.
