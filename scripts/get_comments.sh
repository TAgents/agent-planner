...
echo "Getting comments for node: $NODE_ID in plan $PLAN_ID..."
RESPONSE=$(curl -s -X GET "$API_URL/plans/$PLAN_ID/nodes/$NODE_ID/comments" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN")

# Check for errors
if [[ $RESPONSE == *"error"* ]]; then
  echo "Error getting comments. Response:"
  echo $RESPONSE
  exit 1
else
