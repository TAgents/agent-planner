...
// Node status enum
const nodeStatus = z.enum(['not_started', 'in_progress', 'completed', 'blocked', 'cancelled', 'plan_ready', 'archived'], {
  errorMap: () => ({ message: 'Status must be one of: not_started, in_progress, completed, blocked, cancelled, plan_ready, archived' })
});
...
const updateNode = z.object({
  ...
  metadata: metadata,
  task_mode: z.enum(['research', 'plan', 'implement', 'free']).optional().describe('RPI workflow mode'),
  coherence_status: coherenceStatus.optional().describe('Coherence state (engine-managed; advanced override). Writes use internal values; reads return plain-language values.'),
